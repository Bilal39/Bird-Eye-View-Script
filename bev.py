...
def knn_classifier(dataset, couple_arrays, flag):
    ...
    print("The overall accurac = ", metrics.accuracy_score(y_test, y_pred))
...
def second_level_onward_branches(parent_couple_array, parent_couple_proba, flag, parent_couple_name, covered_nbr_branches=0):
    ...
    for f in range(array_limit):
        ...
        print("\n###################################################################")
        print(
            "Generating new values pair in order to make new generated array :", couple_name)
...
def saving_best_features(dataset, couple_arrays):
    # Combining couples pair to form one single list to apply on the dataset
    # keeping Dataset having '1' while dropping dataset having '0'
    # Couples_arrays is a dictionary
    ...
    # Now dividing dataset into training & testing data along with labels
    ...
    # appending random values to header_list
    ...
    x.insert(loc=0, column=None, value=y)
    ...
    print("The accuracy on best features = ",
          metrics.accuracy_score(y_test, y_pred))

    return metrics.accuracy_score(y_test, y_pred), number_of_features
...
m = 36                      # after 'm' branches, selecting top 'x' branches
...
while repeat_experiment < max_experiments:
    ...
    classes = dict(data_df[0].value_counts())
    for class_name, samples_number in classes.items():
        
        # Checking number of samples in each class
        if samples_number < 10:
            # Moving single sample to testing data
            data_df = data_df.append(
                pd.Series(data_df.loc[data_df[0] == class_name].iloc[0]), ignore_index=True)
            # Deleting the sample from training data( AS ALREADY MOVED TO TESTING DATA)
            data_df.drop(
                index=data_df.loc[data_df[0] == class_name].index[0], axis=0, inplace=True)
        else:
            # If number of samples are more than 10 the selecting 10 percent as testing data
            for n in range(int(abs(samples_number/10))):
                # Moving single sample to testing data
                data_df = data_df.append(
                    pd.Series(data_df.loc[data_df[0] == class_name].iloc[0]), ignore_index=True)
                # Deleting the sample from training data( AS ALREADY MOVED TO TESTING DATA)
    ...
    try:
        while stage < stage_limit:
            ...
            data = pd.read_csv(
                "{}/samples_data2.txt".format(dir_path), header=None)
            data_df = pd.DataFrame(data)
            ...
            if nbr_of_feat % 2 > 0:
                ...
                header_list = []
                for n in range(len(data_df.columns)):
                    header_list.append(n)
                data_df.columns = header_list
                # Adding 2nd last duplcate column
                data_df.insert(nbr_of_feat - 1, None, (data_df.iloc[:, -3]))
                data_df.to_csv(
                    "{}/samples_data2.txt".format(dir_path), index=False, header=None)
                data = pd.read_csv(
                    "{}/samples_data2.txt".format(dir_path), header=None)
                data_df = pd.DataFrame(data)
                nbr_of_feat += 1
            ...
            while k < j:
                ...
                if len(couples_arrays_dict.keys()) >= m:
                    ...
                    print("\nTop {} best arrays based on accuracies = ".format(x),
                          sorted_couples_acc)
                    counter_nbr_save = 0
                    ...
                    for element in sorted_couples_acc:
                        ...
                        if counter_nbr_save <= 1:
                            saving_best_features(
                                data_df, temp_couples_arrays_dict[element[0]])
            ...
            for n in range(len(features.columns)):
                header_list.append(n)
            # Adding header in the csv file
            features.columns = header_list
            # inserting lables columns in the updated features list
            features.insert(loc=0, column=None, value=labels)
            # Removing headers
...
print("------------------------------ R E S U L T -----------------------------")
print("------------------------------------------------------------------------")
print("\nBest accuracies of all the experiments = ")

# Iterate over the keys in dictionary, access value & print line by line
for key in top_acc_all_experiments:
    print(key, ' : ', top_acc_all_experiments[key])
print("\nNumber of Features of best accuracies of all experiments = ")

# Iterate over the keys in dictionary, access value & print line by line
for key in number_of_feat_all_exp:
    print(key, ' : ', number_of_feat_all_exp[key])
print("\n")
